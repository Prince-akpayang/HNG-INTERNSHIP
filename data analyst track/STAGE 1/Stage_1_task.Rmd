...
# 1. Load the Data
We load the product dataset into an R dataframe called `product_df` from a CSV file. This step verifies that the data is correctly imported for further processing.

```{r load-data}
product_df <- read_csv("~/CODE/ANALYSIS/pde/Etini stuff/hng12 stuff/stage1/Productdata.csv")
head(product_df)
```

# 2. Dataset Structure
Here, we explore the structure of the dataset using `glimpse()` to understand its dimensions and variable types. This helps us identify which columns need cleaning and further processing.
...
# 4. Duplicates Check
We count duplicate rows to ensure that each record in the dataset is unique. Removing duplicates is essential for maintaining data accuracy during analysis.
...
```{r rename-columns}
colnames(product_df) <- c("id", "title", "bullet_point", "description", "product_type_id", "product_length")
colnames(product_df)
```

# 6. Handle Missing Values
We remove rows where critical columns (`product_type_id` or `product_length`) are missing and replace missing values in character columns with "Unknown". This ensures that the dataset is complete and reliable for further analysis.
...
We create a new feature, `short_title`, by summarizing the original product title. The function `shorten_text()` splits the title into sentences, applies text ranking if multiple sentences exist, and truncates the result to 50 characters for conciseness.
...
write_csv(product_df, "cleaned_product_data.csv")
```
...
✅ Removed rows with missing critical values and replaced missing texts with unknown
✅ Summarized text fields (the title column specifically) into meaningful short version  
✅ Saved the clean dataset for further use  

The output file **`cleaned_product_data.csv`** is now ready for analysis.  


## Appendix
### Examples of Original and Short Titles
```{r title-examples}
title_examples
```

### Missing Values Before Cleaning
```{r missing-values-before}
missing_values
```
